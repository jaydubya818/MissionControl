...
| Factory incident response | Security + SRE | **Experimental; deterministic control plane implemented** | [Incident command](../software-factory/factory-incident-command.md) | Append-only incident lifecycle, separate containment/restoration authority, distinct command/effect evidence, signed agent detect/propose capabilities, and threat-drill catalog are implemented; no real named-commander pilot incident is claimed | Complete the real pilot preflight drill with canonical command and observed-effect receipts |
