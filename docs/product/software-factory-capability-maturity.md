...
| Factory incident response | Security + SRE | **Production-qualified; one exact repository operation** | [Production qualification](../testing/evidence/factory-incident-command-production-v55/README.md) | Exact `jaydubya818/MissionControl` pause/resume only; other controls, arbitrary repositories, customer incidents, and autonomous emergency authority remain unqualified | Requalify each additional control and target independently before expanding authority |
...
| Governed inference accounting | ML/AI + Platform | **Experimental; default off** | [Bounded Phase 5 completion](../testing/evidence/governed-inference-phase5/completion-record.md); [current program](../software-factory/capability-convergence-program.md) | Aggregate reservations, finite dispatch allowance, immutable observations, settlement correction and durable recovery are qualified offline; live provider billing, two-route comparison and real WorkOrder economics remain unqualified | Supply approved AWS identity and a separately authorized bounded live call, then qualify attributable billing and two exact routes before any route or cost maturity promotion |
