...
profile='fdlc-mutation-feasibility-v1'
# Allowlist only. No broad /tmp write and no deny-pattern blacklist.
fs={':minimal':'read',':workspace_roots':{'.':'write'},str(t):'write',str(native.parent.parent):'read'}
inline=lambda d:'{'+','.join(json.dumps(k)+'='+ (inline(v) if isinstance(v,dict) else json.dumps(v)) for k,v in d.items())+'}'
configs=[f'permissions.{profile}.filesystem='+inline(fs),f'permissions.{profile}.network.enabled=false']
