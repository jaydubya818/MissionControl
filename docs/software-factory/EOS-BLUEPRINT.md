...
| Verification receipts | **Implemented on `main`** | `verificationReceipts` (per-criterion, per-run, artifact-linked, waiver-gated, staleness via revisions/reopen/governance windows) — commits `4dd7423` (PR #19), `aed29f7` (PR #24); `lib/workOrderRevision.ts`, `workOrderGovernance.ts`; governed `accept` refuses without fresh PASSED/WAIVED receipts | `mc` cmds, orchestration routes | 3 test files | Landed by a parallel session AFTER this program's branch base — the sf/* chain must rebase onto it (§26, §32) |
...
1. **RESOLVED during review, replaced by a reconciliation decision:** verification receipts, revisions, reopen/supersede, and governance policies are merged on `main` (PRs #19/#24, commits `4dd7423`/`aed29f7`) — landed by a parallel session after this program's branch base (`4302893`). The real question: rebase the sf/* chain onto current main and reconcile two overlapping work-order vocabularies — the receipts model's `accept` command + per-criterion receipts (main) vs the executor contract's `recordVerificationEvidence` criteria updates (sf/21a). Recommendation: receipts win (richer: run-linked, artifact-linked, staleness-aware); `recordVerificationEvidence` becomes a receipt writer; rework-rate metric unblocks immediately via `workOrderRevisions`/`reopenDecisions`.
