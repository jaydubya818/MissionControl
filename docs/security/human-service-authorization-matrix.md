...
| Persona access profiles | `accessProfiles.getMyAccessContext`, administration queries, profile lifecycle, assignment, rollout mode; exact-only migration | Human UI and trusted operator-run migration | Exact Clerk membership plus `accessProfiles.manage`; immutable revisions, scope ceilings, optimistic concurrency, final-Admin protection | Control plane enforced; route coverage remains inventoried and tenant enforcement is blocked until every configured area passes its server/browser gate |
