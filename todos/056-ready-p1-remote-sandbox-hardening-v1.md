...
- [ ] Freeze exact prior certifications, exe.dev profile, harness identity, and known limits without modifying historic evidence.
- [ ] Verify whether exe.dev provides enforceable deny-by-default egress; prove allowed and denied cases or record the provider limitation.
- [ ] Pin and bind an immutable execution image/toolchain digest where supported; no silent latest.
- [ ] Verify expected versus observed toolchain identity before execution and fail closed on mismatch.
- [ ] Remove runtime Codex installation if a pinned image is feasible; otherwise document and measure the limitation.
- [ ] Preserve bounded, secret-redacted diagnostics for crash, model failure, process termination, timeout, and cancellation without manufacturing success.
- [ ] Prove dedicated workspace permissions, no unnecessary mounts, temporary cleanup, and no cross-Attempt artifact visibility where testable.
- [ ] Prove sandbox code cannot access management, GitHub, acceptance, provider-admin, or Mission Control service credentials.
- [ ] Reconfirm resource isolation, one Attempt per resource, process-group cancellation, and exact teardown.
- [ ] Run controlled network, credential, filesystem, stale-credential, toolchain, prior-artifact, and process-visibility negative tests.
...
- [ ] Create an explicit versioned hardened profile only if warranted by actual enforcement evidence.
- [ ] Re-evaluate GREEN/YELLOW eligibility; do not expand RED.
...
- [ ] Record model, provider, and image/storage costs; preserve unknowns as `null`.
- [ ] Re-run compatibility and the complete requested qualification matrix.
- [ ] Publish immutable evidence under `docs/testing/evidence/remote-sandbox-hardening-v1/` with final VM inventory zero.
- [ ] Obtain fresh GitHub CI and Vercel results for durable changes.
- [ ] Return exactly one permitted final decision and use no hardening claim unsupported by network, toolchain, credential, teardown, and isolation evidence.
