        ...
        <div className="shrink-0 border-b border-line px-3 py-2">
          ...
          {showFabProfile && projectId ? (
            <div className="mb-2 space-y-2 rounded-lg border border-line bg-surface-1 p-2.5 text-[11px]">
              ...
              <div className="flex items-center justify-between"><span className="text-[10px] text-ink-muted">Private to your signed-in identity. Do not store secrets.</span><button type="button" onClick={() => { setProfileSaved(false); setSubmitError(null); void saveFabProfile({ projectId, ...profileDraft }).then(() => setProfileSaved(true)).catch((error) => setSubmitError(error instanceof Error ? error.message : "Fab could not save your profile.")); }} className="rounded bg-registry-accent px-2.5 py-1.5 font-medium text-white">{profileSaved ? "Saved" : "Save"}</button></div>
            </div>
          ) : null}
