...
import { assertProviderPrice, liabilityDigest, type ProviderPrice, type ProviderUsage } from "../../../convex/lib/providerLiability.js";
...
export function createFabBedrockBrokerFactory(
  ...
  assertProviderPrice(config.price, Date.now());
  if (!config.reservationId || !/^sha256:[a-f0-9]{64}$/.test(config.priceDigest)
    || config.priceDigest !== liabilityDigest(config.price)
    || config.price.provider !== "aws-bedrock" || config.price.model !== config.route.modelId
    || config.price.api !== "INVOKE_MODEL" || config.price.inputBound !== "CONSERVATIVELY_BOUNDED"
    || config.price.maximumInputTokens !== config.route.maximumContextTokens
    || !Number.isSafeInteger(config.maximumOutputTokens) || config.maximumOutputTokens < 1 || config.maximumOutputTokens > 4096
    || config.maximumOutputTokens > config.price.maximumOutputTokens
    ...
    return {
      ...
      invoke: async (request: BedrockBrokerRequest, signal: AbortSignal) => {
        ...
        const preSendInputBound = bedrockPreSendInputBound(
          config.route,
          wire,
          config.price.maximumPayloadBytes,
        );
