...
import {
  offlineSandboxEligible,
  assertLocalSandboxScope,
  LOCAL_SANDBOX_SCHEMA,
  localQualificationExecutionPolicy,
...
const executionBackend = v.union(v.literal("persistent-worker"), v.literal("remote-sandbox"), v.literal("isolated-container"));
...
export const registerVersion = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const offline = args.executionBackend === "isolated-container";
    if (offline ? Boolean(args.modelCatalogId || args.toolGrantId) : !args.modelCatalogId) throw new Error("Execution class and inference/tool authority do not match.");
    ...
    if ((args.executionBackend !== "persistent-worker") !== Boolean(args.sandboxProfileId)) {
    ...
    const [modelRoute, sandboxProfile, toolGrant] = await Promise.all([
      args.modelCatalogId ? ctx.db.get(args.modelCatalogId) : Promise.resolve(null),
    ...
    if (!offline && (!modelRoute?.projectId || modelRoute.projectId !== args.projectId
    ...
    if (offline) await assertLocalSandboxScope(ctx, sandboxProfile?.immutableSnapshot, now, access.actorId, {
      projectId: args.projectId,
      tenantId: access.project.tenantId,
    });
    if (offline && (!sandboxProfile || sandboxProfile.projectId !== args.projectId || !offlineSandboxEligible(sandboxProfile, now))) throw new Error("Offline profile requires current separately admitted isolation evidence.");
    ...
    if (args.executionBackend !== "isolated-container" && !modelRouteEligibleForNewFactoryVersion(modelRoute, {
    ...
    const buildSnapshot = (version: number) => executionProfileSnapshot({
      ...
      executionBackend: args.executionBackend,
      offlinePolicy: offline ? ((sandboxProfile!.immutableSnapshot as any)?.schema === LOCAL_SANDBOX_SCHEMA
        ? localQualificationExecutionPolicy(sandboxProfile, isolationModes, now)
        : isolatedPolicy(sandboxProfile!)) : undefined,
...
export const qualify = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const offline = profileSnapshot.schema === OFFLINE_EXECUTION_PROFILE_SCHEMA;
    if (offline) await assertLocalSandboxScope(ctx, admission.sandboxProfile?.immutableSnapshot, now, access.actorId, {
      projectId: profile.projectId,
      tenantId: profile.tenantId,
    });
    const localQualification = offline
      && admission.sandboxProfile?.immutableSnapshot?.schema === LOCAL_SANDBOX_SCHEMA;
    const routeScope = localQualification
      ? (admission.sandboxProfile?.admissionSnapshot as Record<string, any> | undefined)?.scope
      : offline
        ? { workloadClasses: ["SOFTWARE_CHANGE"], riskClasses: ["GREEN"] }
        : profileSnapshot.modelRoute?.qualificationSnapshot?.scope;
    if (!uniqueBoundedEnumScope(args.workloadClasses)
      ...
      throw new Error("Execution Profile qualification scope must be unique and remain within its exact admitted execution scope.");
    }
    if (profile.executionBackend === "remote-sandbox" || offline) {
    ...
    await ctx.db.patch(profile._id, {
      ...
      admissionStatus: offline ? "OFFLINE_ELIGIBLE" : "PRODUCTION_PILOT_ELIGIBLE",
...
function isolatedPolicy(record: { _id: unknown; immutableSnapshot: unknown; profileDigest: string; promotedAt?: number; admissionDigest?: string }): OfflineExecutionPolicy {
  const snapshot = record.immutableSnapshot as IsolatedSandboxSnapshot;
  if (record.promotedAt === undefined || !record.admissionDigest) throw new Error("Isolation admission missing");
  return {
    schema: "factory-offline-execution-policy/v1",
    bridge: { id: "isolated-invocation", version: "1", implementationDigest: snapshot.bridgeDigest, invocationSchema: "factory-isolated-invocation/v2", resultSchema: "factory-isolated-result/v2" },
    backend: { id: "docker-chroot-offline", version: "1", implementationDigest: snapshot.backendDigest, environment: "LOCAL_CONTAINER" },
    isolation: { profileId: String(record._id), profileDigest: record.profileDigest, evidenceDigest: snapshot.qualification.evidenceDigest, admissionDigest: record.admissionDigest, qualifiedAt: record.promotedAt, validUntil: snapshot.qualification.validUntil },
    ...
    capabilities: ["render-markdown", "synthetic-receipt"],
  };
}
