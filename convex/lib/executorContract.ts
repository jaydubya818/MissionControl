...
export function mapBridgeState(input: BridgeStateMappingInput): BridgeStateMappingResult {
  switch (input.bridgeState) {
    ...
    case "succeeded": {
      // Executor only runs a work order after its approval gate cleared at
      // dispatch, so approval is settled here. Under main's receipts model
      // COMPLETED maps to AWAITING_VERIFICATION — DONE is derived only by
      // recordVerificationEvidence (verification precedes completion).
