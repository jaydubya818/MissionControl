...
export const generateCEOBriefData = internalQuery({
  ...
  handler: async (ctx) => {
    ...
    for (const project of projects) {
      ...
      const byStatus = (status: string) => stats.filter(t => t.status === status).length;
...
export const prepareDailyCEOBrief = internalMutation({
  ...
  handler: async (ctx): Promise<{ success: boolean; message: string }> => {
    const briefData: any = await ctx.runQuery(internal.telegram.generateCEOBriefData);
    const message: string = await ctx.runQuery(internal.telegram.formatCEOBrief, { briefData });
    ...
    await ctx.db.insert("notifications", {
      agentId: null as any, // System notification
      type: "SYSTEM",
