  ...
  handler: async (ctx, args) => {
    ...
    if (args.operation) {
      filtered = filtered.filter((a) => 
        a.metadata?.operation === args.operation
      );
    }
...
export const getAuditLog = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.startTime) {
      activities = activities.filter((a) => (a as any)._creationTime >= args.startTime!);
    }
    if (args.endTime) {
      activities = activities.filter((a) => (a as any)._creationTime <= args.endTime!);
    }
    if (args.actorType) {
  ...
  handler: async (ctx, args) => {
    ...
    */
    ...
    for (const activity of activities) {
      const date = new Date((activity as any)._creationTime).toISOString().split("T")[0];
    ...
    for (const [date, dateActivities] of Object.entries(byDate).sort((a, b) => b[0].localeCompare(a[0]))) {
      ...
      for (const activity of dateActivities) {
        const time = new Date((activity as any)._creationTime).toLocaleTimeString();
