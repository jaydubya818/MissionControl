...
import { InMemoryRemoteSandboxJournal, RemoteSandboxRuntime } from "../apps/orchestration-server/src/remoteSandboxRuntime.ts";
...
const evidenceDirectory = path.join(repoRoot, "docs/testing/evidence/production-factory-pilot-v1");
const runResultsPath = path.join(evidenceDirectory, "run-results.json");
const expectedBaseline = "75981d8ae1bd49e235cc1478bac3d0f853fc717f";
...
async function main() {
  ...
  let dataset: any = {
    ...
    preflightFailures: [{
      id: "preflight-local-codex-model-cache-schema-mismatch",
      observedAt: "2026-08-18T19:55:57.831935Z",
      stage: "HARNESS_STARTUP",
      failedInvariant: "The production-qualified Codex harness must start from a compatible, bounded runtime home.",
      reproduction: "Codex CLI 0.146.0 exited before inference while parsing a host models cache that lacked base_instructions.",
      recovery: "Use an ephemeral Attempt-scoped CODEX_HOME containing only the existing authentication file; do not inherit the host models cache or user configuration.",
      category: "ENVIRONMENT_REPAIR",
      avoidableOperationalToil: true,
  ...
  if (options.resumeFailed) {
    ...
    dataset.resumeReason = "Resume only failed executions after fail-closed standalone supervisor repair and bounded timeout correction.";
  }
  ...
  for (const [index, entry] of schedule.entries()) {
    ...
    const execution = await runGovernedExecution(entry, options.resumeFailed ? previous : undefined);
  ...
  if (errors.length) throw new Error(`Pilot evidence contract failed: ${errors.join(" ")}`);
...
function parseArguments(args: string[]) {
  ...
  if (resumeFailed && (limit !== undefined || noRemote)) throw new Error("--resume-failed cannot be combined with --limit or --no-remote.");
  return { limit, noRemote, resumeFailed };
}

async function runGovernedExecution(entry: ScheduleEntry, previous?: ExecutionRecord): Promise<ExecutionRecord> {
  ...
  for (let recoveryAttempt = 1; recoveryAttempt <= 2; recoveryAttempt += 1) {
    ...
    const attemptId = `attempt-${entry.executionId}-${attemptNumber}`;
    const attemptStartedAt = Date.now();
    const attempt: any = {
      attemptId,
      attemptNumber,
      retryOfAttemptId: attempts.at(-1)?.attemptId ?? null,
      worker: workerIdentity(entry),
      lease: {
        id: `lease-${entry.executionId}-${attemptNumber}`,
        current: true,
    ...
    try {
      const execution = entry.backend === "remote-sandbox"
        ? await runRemoteCodex(entry, lineage, repositoryRoot, sourceSha, attempt, attemptNumber)
      ...
      if (execution.status !== "COMPLETED") throw new Error(execution.error ?? `Harness returned ${execution.status}.`);
...
async function runRemoteCodex(
  ...
  const manifest = {
    ...
    causation: {
      ...
      workflowRunId: `workflow-${entry.executionId}-${attemptNumber}`,
    },
    repository: {
      repository: `sellerfi-pilot/${entry.workload.key}`,
      baseSha: sourceSha,
    ...
    intent: { title: entry.workload.title },
    harness: {
      adapter: "codex",
      version: "v1",
      harnessId: "codex",
  ...
  try {
    const result = await runtime.execute({
      projectId: "production-factory-pilot-v1",
      workOrderId: lineage.workOrderId,
      workOrderRevisionNumber: 1,
      workflowRunId: manifest.causation.workflowRunId,
...
function buildGovernedLineage(entry: ScheduleEntry, startedAt: number) {
  ...
  const qualityContract = compileApprovedPlanQualityContract({
    ...
    businessContext: "Production Factory Pilot V1 disposable operational workload.",
    ...
    specLineage: {
      ...
      projectConstitutionRevisionId: "constitution-production-factory-pilot-v1",
  ...
  return {
    constitutionRevisionId: "constitution-production-factory-pilot-v1",
...
function productionPilotSpec(entry: ScheduleEntry) {
  ...
  return {
    ...
    sources: [{ id: "SOURCE-001", kind: "DOC", label: "Production Factory Pilot V1", location: "docs/plans/2026-08-18-production-factory-pilot-v1-plan.md" }],
    clarifications: [],
...
function executionPrompt(entry: ScheduleEntry, attemptNumber: number) {
  return [
    ...
    attemptNumber > 1 ? "This is a new recovery Attempt. Correct the prior failure while preserving scope." : "This is the first Attempt.",
    "Finish with only a JSON object containing: status, summary, completedAcceptanceCriterionIds, incompleteAcceptanceCriterionIds, unknownAcceptanceCriterionIds, verificationCommands, knownRisks, and nextAction.",
    `Use these criterion IDs: ${entry.workload.acceptanceCriteria.map((criterion: any) => criterion.id).join(", ")}.`,
  ].join("\n\n");
}

function workerIdentity(entry: ScheduleEntry) {
  return {
    workerId: entry.backend === "remote-sandbox" ? "worker-production-exedev-n1" : "worker-production-local-codex",
    sessionId: `session-production-pilot-${entry.backend}`,
    generation: 1,
    current: true,
    maxConcurrentRuns: 1,
  };
...
function remoteProfile() {
  return {
    ...
    profileKey: "exe-production-pilot-n1",
    version: 1,
    provider: "EXE_DEV",
    providerProfile: "individual-small",
    providerProfileVersion: "live-certification-v1",
...
function finalizeRoutingShadow(shadow: any, executions: any[]) {
  ...
  return {
    ...
    betterDecisionAssessment: remoteLatency.length && median(localLatency)! < median(remoteLatency)!
      ? "The advisory local recommendation would have reduced median latency for the bounded remote samples; reliability was equivalent where both verified."
...
function buildFactoryLearning(executions: any[], failureInjections: any[]) {
  ...
  const actualRecoverySignals = executions.flatMap((execution, executionIndex) => execution.attempts
    ...
    .map(({ attempt, attemptIndex }: any) => ({
      ...
      deterministicKey: "remote-sandbox operational instability requiring bounded recovery",
      ...
      confidence: 1,
      severity: "HIGH",
      reason: `Remote Attempt required recovery: ${attempt.failureReason}`,
      acceptanceAuthority: false as const,
    })));
  ...
  return {
    ...
    strongestExperimentProposal: strongest ? {
      id: "experiment-proposal-production-pilot-v1",
      candidate: strongest,
      status: "PROPOSED",
      autoPromote: false,
      releasedWorkOrders: [],
      nextStep: "Run a governed experiment comparing the repaired supervisor and five-minute remote profile against the frozen pilot failures; human approval remains required before promotion.",
    } : null,
    acceptanceAuthority: false,
  };
}
...
function buildOperationalMetrics(executions: any[]) {
  ...
  return {
    ...
    firstPassVerificationRate: rate(executions.filter((item) => item.firstPassSuccess).length, executions.length),
...
function failedExecutionRecord(entry: ScheduleEntry, lineage: any, attempts: any[], startedAt: number) {
  return {
    ...
    retries: Math.max(0, attempts.length - 1),
    verification: attempts.at(-1)?.verification ?? { verdict: "NOT_VERIFIED" },
    evidenceCompleteness: 0,
    review: { correctionRequired: null },
    cleanup: attempts.at(-1)?.cleanup ?? { observed: false, passed: null },
    cost: { observed: false, modelUsd: null, infrastructureUsd: null, totalUsd: null },
    metrics: { totalCycleMs: Date.now() - startedAt },
  };
}
