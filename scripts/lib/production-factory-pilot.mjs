export const PILOT_SCHEMA = "production-factory-pilot/v2";
...
export function buildPilotExecutionPrompt(workloadPrompt, acceptanceCriteria, attemptNumber) {
...
export function validatePilotDataset(dataset) {
  ...
  const remote = (dataset?.executions ?? []).filter((item) => item.backend === "remote-sandbox");
  if (remote.length < 3 || remote.some((item) => !item.eventualSuccess)) errors.push("At least three comparable Remote Sandbox executions must succeed.");
  if ((dataset?.executions ?? []).some((item) => item.terminalStructuredResult !== true)) errors.push("Every execution requires a valid terminal structured result.");
  return errors;
}
